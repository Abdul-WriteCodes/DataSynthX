...
def make_heatmap_html(corr_df: pd.DataFrame, title: str) -> str:
    """Build a responsive SVG heatmap that renders correctly inside st.html / st.columns.

    Key design decisions:
    - SVG uses width="100%" so it fills the iframe width rather than overflowing/clipping.
    - viewBox drives the intrinsic coordinate system; aspect-ratio CSS keeps it square.
    - Cap raised to 30 columns (enough for most real-world datasets).
    - Cell size is computed from viewBox space so labels and values stay legible.
    """
    MAX_COLS = 30
    ...
    # ── Coordinate system (viewBox units, not screen px) ────────────────────
    LABEL_OFFSET = 72          # space for axis labels on left and top
    CELL         = max(18, min(48, (600 - LABEL_OFFSET) // n))
    vb_size      = CELL * n + LABEL_OFFSET   # viewBox width = height (square matrix)
    ...
    # ── Axis labels ──────────────────────────────────────────────────────────
    max_lbl  = max(4, CELL // 8)
    font_lbl = max(7, min(10, CELL // 5))
    labels_x = "".join(
        f'<text x="{j * CELL + LABEL_OFFSET + CELL // 2}" y="{LABEL_OFFSET - 6}" '
        f'text-anchor="middle" font-size="{font_lbl}" fill="#6b6b80" '
        f'font-family="Syne,sans-serif" '
        f'transform="rotate(-40 {j * CELL + LABEL_OFFSET + CELL // 2} {LABEL_OFFSET - 6})">'
        f'{c[:max_lbl]}</text>'
        for j, c in enumerate(cols)
    )
    labels_y = "".join(
        f'<text x="{LABEL_OFFSET - 5}" y="{i * CELL + LABEL_OFFSET + CELL // 2 + 4}" '
        f'text-anchor="end" font-size="{font_lbl}" fill="#6b6b80" font-family="Syne,sans-serif">'
        f'{c[:max_lbl]}</text>'
    ...
    # width="100%" + explicit aspect-ratio makes the SVG fill its column and
    # never be clipped by the st.html iframe — this is the fix for blank cards.
    return (
        ...
        f'<svg width="100%" viewBox="0 0 {vb_size} {vb_size}" '
        f'style="aspect-ratio:{vb_size}/{vb_size};display:block;min-width:180px;" '
