...
/* Reset Analysis button — secondary/outline style */
...
def make_heatmap_html(corr_df: pd.DataFrame, title: str) -> str:
    """Build an SVG heatmap that reliably renders inside st.html() / st.columns().

    Root cause of blank cards: SVG needs BOTH explicit width AND height in pixels.
    width="100%" with no height = 0px rendered height in browser iframes.
    aspect-ratio CSS does not work inside Streamlit's sandboxed st.html iframes.
    Solution: compute pixel dimensions directly and set both width and height on the SVG.
    """
    MAX_COLS = 30
    ...
    # ── Pixel dimensions — sized to fit a Streamlit half-width column ───────
    # A Streamlit column in wide-layout is ~580px. We leave room for the card padding.
    LABEL_OFFSET = 70        # px for axis labels on left + top
    MAX_GRID     = 480       # max px for the cell grid area
    CELL         = max(16, min(46, MAX_GRID // n))
    svg_px       = CELL * n + LABEL_OFFSET   # both width and height (square)

    # ── Cells ────────────────────────────────────────────────────────────────
    cells_html = ""
    for i, row_name in enumerate(cols):
        for j, col_name in enumerate(cols):
            val = float(corr_df.loc[row_name, col_name])
            ...
            x = j * CELL + LABEL_OFFSET
            y = i * CELL + LABEL_OFFSET
            val_label = (
                f'<text x="{x + CELL // 2}" y="{y + CELL // 2 + 4}" '
                f'text-anchor="middle" font-size="{max(7, CELL // 5)}" fill="{text_col}" '
                f'font-family="monospace">{val:.2f}</text>'
                if CELL >= 32 else ""
            )
            cells_html += (
                f'<rect x="{x}" y="{y}" width="{CELL - 2}" height="{CELL - 2}" '
                f'rx="2" fill="{bg}" opacity="0.88"/>' + val_label
            )

    # ── Axis labels ──────────────────────────────────────────────────────────
    max_lbl  = max(4, CELL // 7)
    font_lbl = max(7, min(11, CELL // 4))
    labels_x = "".join(
        f'<text x="{j * CELL + LABEL_OFFSET + CELL // 2}" y="{LABEL_OFFSET - 6}" '
        f'text-anchor="end" font-size="{font_lbl}" fill="#9999b0" font-family="monospace" '
        f'transform="rotate(-45 {j * CELL + LABEL_OFFSET + CELL // 2} {LABEL_OFFSET - 6})">'
        f'{c[:max_lbl]}</text>'
        for j, c in enumerate(cols)
    )
    labels_y = "".join(
        f'<text x="{LABEL_OFFSET - 5}" y="{i * CELL + LABEL_OFFSET + CELL // 2 + 4}" '
        f'text-anchor="end" font-size="{font_lbl}" fill="#9999b0" font-family="monospace">'
        f'{c[:max_lbl]}</text>'
    ...
    truncated_notice = (
        f'<div style="font-family:monospace;font-size:10px;color:#f76a6a;margin-top:8px;">'
        f'Showing first {MAX_COLS} of {n_total} columns</div>'
        if truncated else ""
    )

    # Both width AND height must be explicit integers — no %, no auto, no aspect-ratio.
    # overflow-x:auto on the wrapper handles datasets wider than the column.
    return (
        f'<div style="background:#111118;border:1px solid #2a2a3a;border-radius:12px;'
        f'padding:16px;margin-bottom:8px;">'
        f'<div style="font-family:monospace;font-size:10px;color:#6b6b80;'
        f'letter-spacing:2px;text-transform:uppercase;margin-bottom:12px;">{title}</div>'
        f'<div style="overflow-x:auto;">'
        f'<svg width="{svg_px}" height="{svg_px}" '
        f'viewBox="0 0 {svg_px} {svg_px}" '
...
with tab1:
    ...
    if not profile['correlation_matrix'].empty:
        ...
        st.html(make_heatmap_html(profile['correlation_matrix'], "ORIGINAL DATA — PEARSON CORRELATION"))
...
with tab3:
    if 'trust_metrics' not in st.session_state:
        ...
        if not tm['orig_corr'].empty:
            ...
            with col_a:
                st.html(make_heatmap_html(tm['orig_corr'], "ORIGINAL DATA"))
            with col_b:
                st.html(make_heatmap_html(tm['synth_corr'], "SYNTHETIC DATA"))
