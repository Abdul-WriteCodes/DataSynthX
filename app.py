...
def make_heatmap_html(corr_df: pd.DataFrame, title: str) -> str:
    ...
    return (
        ...
        f'padding:16px;margin-bottom:8px;overflow:hidden;">'
...
with st.sidebar:
    ...
    if st.button("↩ Sign Out", use_container_width=True):
        for k in ["authenticated","access_key","key_owner","key_plan","credits",
                  "synth_df","trust_metrics","gen_status","ai_explanation","ai_use_case"]:
...
{"User's stated use case: " + use_case if use_case else "No use case stated — infer from column names."}"""
                ...
                with st.spinner("Analysing your results…"):
                    try:
                        ...
                        st.session_state["ai_use_case"]    = use_case or "inferred from columns"
        ...
        if st.session_state.get("ai_explanation"):
            ...
            stated_use  = st.session_state.get("ai_use_case", "")
    ...
    if not numeric_cols and not categorical_cols:
        ...
        with st.spinner(""):
            try:
                ...
                st.session_state.pop('ai_use_case', None)
