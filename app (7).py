...
with tab3:
    if 'trust_metrics' not in st.session_state:
        ...
        # SCI Hero — Plotly gauge (SVG inside st.html has zero height in iframes)
        gauge_fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=sci,
            number=dict(
                font=dict(size=52, color=color, family="Syne, sans-serif"),
                suffix="",
            ),
            gauge=dict(
                axis=dict(
                    ...
                    tickfont=dict(size=10, color="#6b6b80"),
                    tickwidth=1,
                    tickcolor="#2a2a3a",
                ),
                ...
                borderwidth=0,
                steps=[
                    dict(range=[0,   50],  color="#0d0d14"),
                    dict(range=[50,  75],  color="#111118"),
                    dict(range=[75, 100],  color="#131320"),
                ],
                threshold=dict(
            ...
            title=dict(
                text=f"STRUCTURAL CONSISTENCY INDEX<br><span style='font-size:11px;color:#6b6b80'>"
                     f"Weighted composite · correlation · distribution · categorical</span>",
                font=dict(size=11, color="#9999b0", family="Space Mono, monospace"),
            ),
        ...
        # Badge centred below gauge
        st.html(f"""
        <div style="text-align:center;margin-top:-12px;margin-bottom:20px;">
            <span class="badge {badge_cls}" style="font-size:11px;padding:5px 18px;">
                {badge_label}
            </span>
        </div>
        """)
